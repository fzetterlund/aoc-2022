...
public static class Part2
{
    public static long Calculate(string[] lines)
    {
        ...
        var usedSpace = directorySummary.OrderByDescending(x => x.Value).First().Value;
        var neededSpace = 30000000 - (70000000 - usedSpace);
        return directorySummary.Where(v => v.Value > neededSpace).Select(x => x.Value).Min();
    }
}
